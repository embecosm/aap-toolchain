...
echo "Key script variables:"
echo "  autocheckout=${autocheckout}"
echo "  autopull=${autopull}"
echo "  doclean=${doclean}"
echo "  builddir=${builddir}"
echo "  installdir=${installdir}"
echo "  symlinkdir=${symlinkdir}"
...
then
    ...
    if ! ${topdir}/toolchain/get-versions.sh ${topdir} ${version_file} \
            ${autocheckout} ${autopull}
    then
        echo "ERROR: Failed to checkout GIT versions of tools."
        exit 1
    fi
else
    echo "Repo manifest detected - not performing git operations"
fi
...
then
    ...
    echo "Linking to built AAP compiler"
    ...
    echo "Linking to installed AAP compiler"
...
then
    ...
    cd ${builddir_gnu}
    if ! make install-binutils install-ld install-gdb
    then
...
then
    ...
    cd ${builddir_newlib}
    if ! make install-target-libgloss install-target-newlib
    then
...
then
    ...
    if ! mkdir -p ${installdir}/aap/lib
    then
    echo "ERROR: Failed to create install dir for Compiler RT"
    exit 1
    fi

    if ! cp libcompiler_rt.a ${installdir}/aap/lib
    then
...
if [ "x${symlinkdir}" != "x" ]
then
    echo "Setting symbolic link to install directory"
